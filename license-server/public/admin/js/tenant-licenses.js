(function () {
    ...
    tbody.innerHTML = json.licenses.map((l) => {
      ...
      <tr>
        ...
        <td class="px-4 py-3 space-x-2 text-sm">
          <button data-act="renew-lic" data-key="${escape(l.license_key)}" class="text-indigo-600 hover:text-indigo-700">续期</button>
  ...
  function openCreate() {
    ...
    $('cl-form').onsubmit = async (e) => {
      ...
      if (status >= 400) {
        $('cl-err').textContent = formatErr(json);
