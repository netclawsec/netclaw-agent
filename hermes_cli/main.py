...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ NetClaw Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def cmd_version(args):
    """Show version."""
    print(f"NetClaw Agent v{__version__} ({__release_date__})")
...
""",
    ...
    # Enforce a valid NetClaw Agent license before any command that actually
    # runs the product.  The `license`, `doctor`, `update` and `uninstall`
    # subcommands are self-service escape hatches and remain usable without a
    # valid activation.  Skip via NETCLAW_LICENSE_SKIP=1 for dev / CI.
    _LICENSE_EXEMPT = {"license", "doctor", "update", "uninstall", "version", "logout"}
    if (args.command or "chat") not in _LICENSE_EXEMPT and os.getenv(
        "NETCLAW_LICENSE_SKIP"
    ) != "1":
        try:
            _license.enforce()
        except _license.LicenseError as err:
            print(f"license error: {err}", file=sys.stderr)
            print("\nRun:  netclaw license activate <LICENSE-KEY>", file=sys.stderr)
            print(
                "or:   NETCLAW_LICENSE_SKIP=1 netclaw ...   (dev bypass)",
                file=sys.stderr,
            )
            sys.exit(2)
