...
const LOCALES = {
  en: {
    ...
    cmd_skills: 'List available NetClaw Agent skills',
    ...
    settings_desc_cli_sessions: 'Merges sessions from the NetClaw Agent CLI (state.db) into the session list. Click a CLI session to import it and continue the conversation.',
    ...
    settings_desc_bot_name: 'Display name for the assistant throughout the UI. Defaults to NetClaw Agent.',
    ...
    onboarding_title: 'Welcome to NetClaw Agent Web UI',
    onboarding_lead: 'A quick guided setup will verify NetClaw Agent, save a real provider configuration, choose a workspace and model, and optionally protect the app with a password.',
    onboarding_back: 'Back',
    onboarding_continue: 'Continue',
    onboarding_skip: 'Skip setup',
    onboarding_skipped: 'Setup skipped — using existing config.',
    yolo_label: 'YOLO',
    yolo_title_on: 'YOLO on — every command auto-approved for this session. Click to disable.',
    yolo_title_off: 'YOLO off — commands still require approval. Click to enable.',
    yolo_toast_on: 'YOLO mode ENABLED — commands will auto-approve',
    yolo_toast_off: 'YOLO mode disabled',
    yolo_need_session: 'Start a session first',
    yolo_toggle_failed: 'Failed to toggle YOLO: ',
    onboarding_open: 'Open NetClaw Agent',
    onboarding_step_system_title: 'System check',
    onboarding_step_system_desc: 'Verify NetClaw Agent and config visibility.',
    onboarding_step_setup_title: 'Provider setup',
    onboarding_step_setup_desc: 'Save the minimum NetClaw Agent provider config.',
    ...
    onboarding_notice_system_ready: 'NetClaw Agent looks reachable from the Web UI.',
    onboarding_notice_system_unavailable: 'NetClaw Agent is not fully available yet. Bootstrap can install it, but provider setup may still require a terminal.',
    onboarding_check_agent: 'NetClaw Agent',
    ...
    onboarding_notice_setup_required: 'Choose a simple provider path here. Advanced OAuth flows still belong in the NetClaw Agent CLI for now.',
    onboarding_notice_setup_already_ready: 'A working NetClaw Agent provider setup is already detected. You can keep it or replace it here.',
    onboarding_oauth_provider_ready_title: 'Provider already authenticated',
    onboarding_oauth_provider_ready_body: 'This instance is configured to use an OAuth provider (<strong>{provider}</strong>) that was set up via the NetClaw Agent CLI. No API key is needed here — click Continue to finish setup.',
    ...
    onboarding_api_key_help_prefix: 'Saved as a secret in your NetClaw Agent .env file using',
    ...
    onboarding_workspace_help: 'Pick the model NetClaw Agent should use for new chats after setup completes.',
  ...
  es: {
    ...
    cmd_skills: 'Listar las skills de NetClaw Agent disponibles',
    ...
    settings_desc_cli_sessions: 'Fusiona las sesiones del CLI de NetClaw Agent (state.db) en la lista de sesiones. Haz clic en una sesión de CLI para importarla y continuar la conversación.',
    ...
    settings_desc_bot_name: 'Nombre visible del asistente en toda la UI. Por defecto es NetClaw Agent.',
    ...
    onboarding_title: 'Bienvenido a NetClaw Agent Web UI',
    onboarding_lead: 'Una guía rápida verificará NetClaw Agent, guardará una configuración real del proveedor, elegirá un espacio de trabajo y un modelo, y opcionalmente protegerá la app con una contraseña.',
    ...
    onboarding_open: 'Abrir NetClaw Agent',
    onboarding_step_system_title: 'Comprobación del sistema',
    onboarding_step_system_desc: 'Verifica NetClaw Agent y la visibilidad de la configuración.',
    onboarding_step_setup_title: 'Configuración del proveedor',
    onboarding_step_setup_desc: 'Guarda la configuración mínima real de NetClaw Agent.',
    ...
    onboarding_notice_system_ready: 'Parece que NetClaw Agent está accesible desde la Web UI.',
    onboarding_notice_system_unavailable: 'NetClaw Agent todavía no está totalmente disponible. Bootstrap puede instalarlo, pero la configuración del proveedor quizá aún requiera una terminal.',
    onboarding_check_agent: 'NetClaw Agent',
    ...
    onboarding_notice_setup_required: 'Elige aquí una ruta simple de proveedor. Los flujos OAuth avanzados siguen siendo del CLI de NetClaw Agent por ahora.',
    onboarding_notice_setup_already_ready: 'Ya se detectó una configuración funcional del proveedor de NetClaw Agent. Puedes conservarla o reemplazarla aquí.',
    onboarding_oauth_provider_ready_title: 'Proveedor ya autenticado',
    onboarding_oauth_provider_ready_body: 'Esta instancia está configurada para usar un proveedor OAuth (<strong>{provider}</strong>) configurado mediante la CLI de NetClaw Agent. No se necesita clave API aquí — haz clic en Continuar para finalizar la configuración.',
    ...
    onboarding_api_key_help_prefix: 'Se guarda como secreto en tu archivo .env de NetClaw Agent usando',
    onboarding_base_url_label: 'Base URL',
    ...
    onboarding_workspace_help: 'Elige el modelo que NetClaw Agent debe usar para nuevos chats cuando termine la configuración.',
  ...
  de: {
    ...
    cmd_skills: 'Verfügbare NetClaw Agent-Skills auflisten',
    ...
    settings_desc_cli_sessions: 'Fügt Sitzungen aus der NetClaw Agent CLI (state.db) in die Sitzungsliste ein. Klicken Sie auf eine CLI-Sitzung, um sie zu importieren und das Gespräch fortzusetzen.',
    ...
    settings_desc_bot_name: 'Anzeigename für den Assistenten in der UI. Standardmäßig NetClaw Agent.',
  ...
  zh: {
    ...
    cmd_skills: '\u5217\u51fa\u53ef\u7528\u7684 NetClaw Agent \u6280\u80fd',
    ...
    settings_desc_cli_sessions: '将 NetClaw Agent CLI（state.db）中的会话合并到会话列表。点击某个 CLI 会话可导入并继续对话。',
    ...
    settings_desc_bot_name: '助手在 UI 中的显示名称。默认为 NetClaw Agent。',
    settings_desc_password: '输入新密码以设置或更改。留空保持当前设置。',
    // onboarding
    onboarding_badge: '首次运行',
    onboarding_title: '欢迎使用 NetClaw Agent Web UI',
    onboarding_lead: '快速引导将验证 NetClaw Agent、保存真实的提供商配置、选择工作区和模型，并可选设置密码保护应用。',
    onboarding_back: '返回',
    onboarding_continue: '继续',
    onboarding_skip: '跳过设置',
    onboarding_skipped: '设置已跳过 — 使用现有配置。',
    yolo_label: '免审批',
    yolo_title_on: '已开启 — 本会话所有命令自动批准，点击关闭',
    yolo_title_off: '已关闭 — 命令仍需审批后执行，点击开启',
    yolo_toast_on: '已开启免审批模式 — 命令将自动通过',
    yolo_toast_off: '已关闭免审批模式',
    yolo_need_session: '请先开始一个会话',
    yolo_toggle_failed: '切换免审批模式失败：',
    onboarding_open: '打开 NetClaw Agent',
    onboarding_step_system_title: '系统检查',
    onboarding_step_system_desc: '验证 NetClaw Agent 与配置可见性。',
    onboarding_step_setup_title: '提供商设置',
    onboarding_step_setup_desc: '保存最小可用的 NetClaw Agent 提供商配置。',
    ...
    onboarding_notice_system_ready: 'NetClaw Agent 看起来可从 Web UI 访问。',
    onboarding_notice_system_unavailable: 'NetClaw Agent 尚未完全可用。Bootstrap 可以安装它，但提供商设置可能仍需要终端。',
    onboarding_check_agent: 'NetClaw Agent',
    ...
    onboarding_notice_setup_required: '请先在此选择一个简单的提供商路径。高级 OAuth 流程暂时仍建议在 NetClaw Agent CLI 中完成。',
    onboarding_notice_setup_already_ready: '已检测到可用的 NetClaw Agent 提供商配置。你可以保留它，或在这里替换。',
    onboarding_oauth_provider_ready_title: '提供商已完成认证',
    onboarding_oauth_provider_ready_body: '此实例已配置为使用通过 NetClaw Agent CLI 设置的 OAuth 提供商（<strong>{provider}</strong>）。这里不需要 API key，点击继续即可完成设置。',
    ...
    onboarding_api_key_help_prefix: '会作为密钥保存到 NetClaw Agent .env 文件中，变量名为',
    onboarding_base_url_label: 'Base URL',
    onboarding_base_url_placeholder: 'https://your-endpoint.example/v1',
    onboarding_base_url_help: '用于 OpenAI 兼容路由、自托管服务、LiteLLM、Ollama、LM Studio、vLLM 或类似端点。',
    onboarding_model_label: '默认模型',
    onboarding_workspace_help: '选择设置完成后 NetClaw Agent 在新聊天中使用的模型。',
    onboarding_custom_model_placeholder: 'your-model-name',
  ...
  'zh-Hant': {
    ...
    settings_desc_cli_sessions: '\u5c07 NetClaw Agent CLI (\u7684 state.db) \u4e2d\u7684\u6703\u8a71\u6dfb\u52a0\u5230\u6703\u8a71\u6e05\u55ae\u3002\u9ede\u64ca\u4e00\u500b CLI \u6703\u8a71\u5c07\u5c0e\u5165\u5b83\u7a0b\u5f0f\u4e26\u7e7c\u7e8c\u5b58\u5c0d\u8a71\u3002',
